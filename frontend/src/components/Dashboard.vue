<template>
  <div class="space-y-6">
    ...
    <div v-if="result" class="grid gap-6 grid-cols-1 lg:grid-cols-12">
      ...
      <!-- Saudi medications: AI picks (primary) + curated catalogue (secondary) -->
      <section class="glass p-5 sm:p-6 lg:col-span-7">
        <div class="flex items-center justify-between mb-3 flex-wrap gap-2">
          <div class="flex items-center gap-2">
            ...
            <div>
              <h3 class="text-base font-semibold">{{ t('ai_picks') }} · {{ t('pharmacy_suggestions') }}</h3>
              <p class="text-[11px] text-slate-500">{{ t('available_at') }}</p>
            </div>
          </div>
          <span v-if="result.medication_cards" class="pill bg-violet-50 text-violet-700 border border-violet-200">
            {{ result.medication_cards.model || t('ai_model_label') }}
          </span>
        </div>

        <!-- AI picks: clean 5-field cards -->
        <div v-if="aiCards.length" class="grid grid-cols-1 lg:grid-cols-2 gap-3">
          <article v-for="(c, i) in aiCards" :key="'ai'+i"
            class="rounded-xl border border-slate-200 bg-white/80 p-4 hover:shadow-soft transition flex flex-col">
            <!-- 1. Name -->
            <div class="flex items-start justify-between gap-3">
              <div class="min-w-0">
                <div class="font-semibold text-slate-900 truncate">{{ isRTL ? c.name_ar : c.name }}</div>
                <div v-if="!isRTL" class="ar text-xs text-slate-500 mt-0.5" dir="rtl">{{ c.name_ar }}</div>
                <div v-else class="text-xs text-slate-500 mt-0.5" dir="ltr">{{ c.name }}</div>
              </div>
              <div class="text-right shrink-0">
                <div v-if="c.meta?.indicative_price_sar" class="text-sm font-bold text-emerald-700">
                  {{ c.meta.indicative_price_sar }} <span class="text-[10px] font-medium text-slate-500">SAR</span>
                </div>
                <span :class="c.meta?.rx_required ? 'pill bg-amber-50 text-amber-700 border border-amber-200' : 'pill bg-emerald-50 text-emerald-700 border border-emerald-200'">
            ...
            <div class="flex flex-wrap items-center gap-1.5 mt-2">
              ...
              <span v-if="c.meta?.evidence_level" class="pill bg-slate-100 text-slate-600 border border-slate-200">
                {{ t('evidence_level') }}: {{ c.meta.evidence_level }}
              </span>
            </div>

            <!-- 2. Description -->
            <p class="mt-3 text-xs text-slate-600">{{ isRTL ? c.description_ar : c.description }}</p>

            <!-- 3. Why -->
            <div class="mt-3">
              <div class="text-[10px] uppercase tracking-wider text-violet-700 font-semibold">{{ t('why_this') }}</div>
              <p class="text-xs text-slate-700 leading-relaxed mt-0.5">{{ isRTL ? c.why_ar : c.why }}</p>
            </div>
            ...
            <ul class="mt-3 space-y-1">
              <li v-for="(b, j) in (isRTL ? c.bullets_ar : c.bullets)" :key="j"
                ...
                <span class="mt-1 h-1 w-1 rounded-full bg-accent shrink-0"></span>
                <span>{{ b }}</span>
              </li>
            </ul>

            <!-- 5. Link + extras -->
            <div class="mt-auto pt-3 flex flex-wrap items-center gap-1.5 border-t border-slate-100">
              <a v-if="c.link" :href="c.link" target="_blank" rel="noopener"
                 class="btn-primary text-xs px-3 py-1.5">
                <svg viewBox="0 0 24 24" class="h-3.5 w-3.5" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M14 3h7v7m0-7L10 14m-3 7H4a1 1 0 01-1-1v-3"/></svg>
                {{ t('view_product') }}
              </a>
              <a v-for="(e, k) in (c.extra_links || []).slice(0, 3)" :key="k"
                 :href="e.url" target="_blank" rel="noopener"
                 class="pill bg-slate-100 text-slate-600 hover:bg-slate-200 text-[10px]">
        ...
        <!-- Ranking signals chip row -->
        <div v-if="result.medication_cards?.ranking_signals?.length" class="mt-3 text-[10px] text-slate-500">
          {{ t('ranking_signals') }}:
          <span v-for="(s, i) in result.medication_cards.ranking_signals" :key="i"
                class="ml-1 px-1.5 py-0.5 rounded bg-slate-100">{{ s }}</span>
        </div>

        <!-- Full curated catalogue (collapsible secondary view) -->
        <div class="mt-5 pt-4 border-t border-slate-200">
          <button class="text-xs text-accent-700 hover:underline" @click="showCatalog = !showCatalog">
            {{ showCatalog ? t('hide_full_catalog') : t('show_full_catalog') }} ({{ filteredMeds.length }})
          </button>
        </div>
        <div v-if="showCatalog" class="mt-3">
